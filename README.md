...
このような処理では、まず処理単位を小さく分けることが有効です。

例えば、ファイルを一定件数ごとのchunkに分けたり、対象範囲をpartitioningして複数スレッドで処理したりすると、単一スレッドで順番に処理するよりも処理時間を短縮できます。
全件を一度にメモリへ持たず、分割した単位ごとに処理できるだけでも、安定性や再実行のしやすさは大きく変わります。

ただ、扱うデータ量や求められる処理時間によっては、それだけでは目標に届かないこともあります。
