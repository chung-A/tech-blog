...
> 画像生成プロンプト:
> 「性能改善を技術名からではなく処理フローから考える」というテーマの、横長の技術ブログ向け図解を作成してください。左から右へ「ユーザー操作 / バッチ起動 / 外部連携」→「処理フロー」→「読む処理」「書く処理」→「ボトルネック特定」へ流れる構成。色は白背景、青と緑を基調、アイコンはシンプル、文字は日本語。強調メッセージは「まず処理フローを見る」。
...
> 画像生成プロンプト:
> 書き込み処理の性能改善を説明する技術ブログ向け図解を作成してください。左側に「同期処理: DB保存・状態確定・成功/失敗応答」、右側に「後続処理: 通知・ログ・集計・外部連携」を配置し、中央に「Queue / Async / Worker」で分離する構成。下部に「同期で確定すべき処理を小さくする」「後続処理は再試行・重複対策が必要」と表示。白背景、青/オレンジ基調、フラットデザイン。
...
> 画像生成プロンプト:
> 読み込み処理の性能改善を説明する技術ブログ向け図解を作成してください。3つのレイヤーに分ける構成: 「Web/Network: Browser Cache, CDN」「Application: Memory Cache, Redis」「DB/Batch: Index, Replication, Partitioning, Cursor/Paging」。それぞれの下に短い判断軸として「毎回処理する必要があるか」「同じデータを繰り返し読むか」「探す範囲が広すぎないか」を表示。白背景、青/緑基調、読みやすい日本語ラベル。
...
> 画像生成プロンプト:
> 「同じ性能問題でも処理の性質によって対策が変わる」ことを説明する技術ブログ向けの概要図を作成してください。4つのカードを横並びまたは2x2で配置: 「メイン画面API」「申込・登録API」「DB大容量バッチ」「ファイル受信バッチ」。各カードに小さく「読む」「書く」「大量データ」「ファイル→Topic→Consumer」と表示。下部に「処理フローを分けて考える」と強調。白背景、落ち着いた青/緑/オレンジ。
...
> 画像生成プロンプト:
> メイン画面表示APIの読み込みボトルネックを説明する図解を作成してください。上部に「ユーザー → メイン画面API」。下に3分類: 1) 「共通・更新頻度低: お知らせ/バナー/イベント情報 → Cache/CDN/Redis」、2) 「ユーザーごと・最新性重視: ポイント/状態/履歴 → DB Query改善(Index, JOIN見直し, 取得件数)」、3) 「計算コスト高: 推薦データ/集計値 → 事前計算テーブル/集計テーブル」。下部に「同じ読む処理でも性質によって対策は変わる」。日本語ラベル、白背景、フラットな技術ブログ図。
...
> 画像生成プロンプト:
> 申込・登録APIの書き込みボトルネックを説明する図解を作成してください。左から「ユーザー申込」→「申込API」。APIの中を2段に分ける: 上段「同期で必要: 入力検証、重複確認、申込保存、状態確定、成功/失敗応答」、下段「後続で可能: 通知、ログ、集計、外部連携」。同期部分からOutbox/Queueへ矢印、Workerが後続処理を実行。下部に「トランザクションを小さくし、後続処理は冪等性を持たせる」。白背景、青/オレンジ基調、日本語ラベル。
...
> 画像生成プロンプト:
> DBから大量データを読み込み、DBへ反映するバッチの性能改善を説明する図解を作成してください。左に「大きな対象データ」。中央で「月別 / 日別 / ID範囲で分割」し、3つのPartition A/B/Cへ分かれる。各Partitionの下に「Cursor / Paging」「Chunk処理」。最後に「Batch Write / Bulk Update」→「DB反映」。下部に「読む単位は小さく、書く単位は適切にまとめる」「再実行しやすい単位で分ける」。白背景、青/緑基調、技術ブログ向けの読みやすい日本語図。
...
> 画像生成プロンプト:
> 大きなファイルを読み込みDBへ反映するバッチで、ファイル読み込みとDB書き込みをTopicで分離する構成の図解を作成してください。左から「Large File」→「File Reader（順次読み込み）」→「Chunk変換」→「Topic / Queue」。Topicの先を「Partition 0」「Partition 1」「Partition 2」に分け、それぞれ「Consumer Worker」→「Batch Write」→「DB」へ流す。補足として「ファイル自体を単純に分割しない」「読み込み速度とDB反映速度を分離」「重複・再試行・冪等性に注意」を下部に表示。白背景、青/紫/緑基調、日本語ラベル。
...
> 画像生成プロンプト:
> 性能改善のトレードオフを説明する技術ブログ向け図解を作成してください。中央に天秤またはバランス図。左側に「速くなること: Cache, Index, Queue, Bulk」、右側に「増えるコスト: 整合性, TTL, 書き込み負荷, 再試行, 重複管理, 運用複雑性」。下部に「性能改善は、どのコストをどこに移すかを考えること」と表示。白背景、青/オレンジ基調、シンプルな日本語ラベル。
...
> 画像生成プロンプト:
> 技術ブログのまとめ用の図解を作成してください。左に「Redis / Kafka / Index などの技術名」、中央に「処理フローを見る」「読む / 書く / 大量データ / 後続処理に分類」、右に「ボトルネックに合った手段を選ぶ」。矢印で左から右へ流し、最終メッセージとして「技術は答えではなく、分類した後に選ぶ手段」と表示。白背景、落ち着いた青と緑、ミニマルなアイコン。
