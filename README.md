...
また、推薦データや集計値のように計算コストが高いものは、リクエストのたびに計算するのではなく、事前に作成したread modelやsummary tableを参照する構成も考えられます。
...
#### 適用例3: 大きなファイルを読み込み、DBに反映するバッチ

大きなファイルを読み込み、内容を検証してDBに反映する処理もあります。
私が見たケースでは、いきなり複雑な構成を選ぶのではなく、処理を段階的に見直していきました。

まず、単一スレッドで順番に処理する方法です。
構成はシンプルですが、ファイルサイズやDB反映件数が大きくなると、どうしても処理時間が伸びやすくなります。

次に、chunkやpartitioningを使って処理単位を小さくする方法です。
ファイルを一定件数ごとのchunkに分けたり、対象範囲をpartitioningして複数スレッドで処理したりすると、単一スレッドで処理するよりも処理時間を短縮できます。
全件を一度にメモリへ持たなくてよくなり、失敗した場合も分割した単位から再実行しやすくなります。
並列処理によって、CPUやDB接続などのリソースをより使いやすくなる点も大きな改善でした。

DBに反映する部分でも、1件ずつinsert/updateするのではなく、chunk単位でbatch writeやbulk updateにまとめることで、DBへの往復回数を減らせます。
このように、大量処理では読む単位を小さくし、書く単位は適切にまとめることが重要になります。

ただ、扱うデータ量や求められる処理時間によっては、それでも目標に届かないことがあります。
私が経験したケースでも、chunkやpartitioningによって一定の改善はできましたが、さらに処理時間を短縮する必要がありました。

そこで、処理フローをもう一段分けて、ファイルを読む処理とDBに反映する処理の間にキューを置く構成を考えました。
...
こうすると、読み込みと書き込みをそれぞれ別の観点で調整しやすくなる場合があります。

もちろん、この方法にも注意点があります。
メッセージの重複処理、再試行、どこまで処理済みかの管理、DB反映時の冪等性、反映結果の検証などを考える必要があります。
そのため、最初から複雑な構成を選ぶというより、chunkやpartitioningで処理単位を小さくしたうえで、それでも読み込みと書き込みの速度差がボトルネックになる場合の選択肢として考えるのが自然だと思います。
